#!/usr/bin/env python3
"""Controlled lambda=1.0 7x7 retraining with phi4 protected."""
...
OUT = LAM_ROOT / "tests/intermediate/7x7_full_retraining_phi4_constrained"
CAND_DIR = LAM_ROOT / "kernels/candidates/7x7_full_retraining_phi4_constrained"
...
def main() -> int:
    ...
    if best is not None:
        ...
        cand = make_candidate(
            ...
            "selection": "best staged controlled from-scratch 7x7 candidate with phi4 protected as a hard guardrail",
    ...
    lines = [
        "# Lambda 1.0 Controlled Full 7x7 Retraining With Phi4 Constraint",
        "",
        "This study retrains the symmetric 7x7 orbit classes without keeping the retrained 5x5 core fixed. Stages 1 and 2 exclude `(3,3)`; stage 3 allows penalized `(3,3)` only if no no-corner candidate beats the current final. This pass adds a hard phi4 guardrail: `phi4 KS <= min(0.018, current_final_phi4_KS + 0.004)`.",
        "",
