#!/usr/bin/env python3
"""Controlled lambda=1.0 7x7 retraining with phi2 and phi4 protected."""
...
OUT = LAM_ROOT / "tests/intermediate/7x7_full_retraining_phi2_phi4_constrained"
CAND_DIR = LAM_ROOT / "kernels/candidates/7x7_full_retraining_phi2_phi4_constrained"
...
def guardrail_penalty(rows: dict[str, dict[str, Any]], mom: dict[str, float], cls: dict[str, float], reference: dict[str, dict[str, Any]]) -> float:
    ...
    for obs, limit in {
        ...
        "phi4": min(0.018, float(reference["phi4"]["ks_statistic"]) + 0.004),
        "NN": 0.035,
        "m2": 0.025,
        "m4": 0.025,
        "G_pmin_avg": 0.035,
    ...
    penalty += 3.0e5 * max(0.0, float(rows["phi4"]["ks_statistic"]) - min(0.018, float(reference["phi4"]["ks_statistic"]) + 0.004)) ** 2
...
def category(rows: dict[str, dict[str, Any]], mom: dict[str, float], reference: dict[str, dict[str, Any]], candidate: str) -> str:
    ...
    passes = (
        ...
        and float(rows["phi4"]["ks_statistic"]) <= min(0.018, float(reference["phi4"]["ks_statistic"]) + 0.004)
...
def main() -> int:
    ...
    if best is not None:
        ...
        cand = make_candidate(
            ...
            "selection": "best staged controlled from-scratch 7x7 candidate with phi2 and phi4 protected as hard guardrails",
    ...
    lines = [
        "# Lambda 1.0 Controlled Full 7x7 Retraining With Phi2 And Phi4 Constraints",
        "",
        "This study retrains the symmetric 7x7 orbit classes without keeping the retrained 5x5 core fixed. Stages 1 and 2 exclude `(3,3)`; stage 3 allows penalized `(3,3)` only if no no-corner candidate beats the current final. This pass adds hard one-site moment guardrails: `phi2 KS <= min(0.083, current_final_phi2_KS + 0.002)` and `phi4 KS <= min(0.018, current_final_phi4_KS + 0.004)`.",
        "",
