#!/usr/bin/env python3
"""Controlled lambda=1.0 7x7 retraining with phi2 priority and NN guard."""
...
OUT = LAM_ROOT / "tests/intermediate/7x7_full_retraining_phi2_nn_guarded"
CAND_DIR = LAM_ROOT / "kernels/candidates/7x7_full_retraining_phi2_nn_guarded"
...
def local_score(rows: dict[str, dict[str, Any]]) -> float:
    return 8.0 * scalar(rows["phi2"]) + 2.0 * scalar(rows["phi4"]) + 3.0 * scalar(rows["local_kurtosis_ratio"])
...
def guardrail_penalty(rows: dict[str, dict[str, Any]], mom: dict[str, float], cls: dict[str, float], reference: dict[str, dict[str, Any]]) -> float:
    ...
    for obs, limit in {
        ...
        "phi2": min(0.078, float(reference["phi2"]["ks_statistic"])),
        "phi4": 0.030,
        "NN": 0.0335,
        "m2": 0.025,
        "m4": 0.025,
    ...
    penalty += 8.0e5 * max(0.0, float(rows["phi2"]["ks_statistic"]) - min(0.078, float(reference["phi2"]["ks_statistic"]))) ** 2
    penalty += 8.0e4 * max(0.0, float(rows["phi4"]["ks_statistic"]) - 0.030) ** 2
    penalty += 3.0e5 * max(0.0, float(rows["NN"]["ks_statistic"]) - 0.0335) ** 2
...
def category(rows: dict[str, dict[str, Any]], mom: dict[str, float], reference: dict[str, dict[str, Any]], candidate: str) -> str:
    ...
    passes = (
        ...
        and float(rows["phi2"]["ks_statistic"]) <= min(0.078, float(reference["phi2"]["ks_statistic"]))
        and float(rows["phi4"]["ks_statistic"]) <= 0.030
        and float(rows["NN"]["ks_statistic"]) <= 0.0335
    ...
    improves = (
        float(rows["phi2"]["ks_statistic"]) <= min(0.078, float(reference["phi2"]["ks_statistic"]))
        and float(rows["phi4"]["ks_statistic"]) <= 0.030
...
def main() -> int:
    ...
    if best is not None:
        ...
        cand = make_candidate(
            ...
            "selection": "best staged controlled from-scratch 7x7 candidate with phi2 prioritized, phi4 relaxed below 0.030, and NN guarded",
    ...
    lines = [
        "# Lambda 1.0 Controlled Full 7x7 Retraining With Phi2 Priority And NN Guard",
        "",
        "This study retrains the symmetric 7x7 orbit classes without keeping the retrained 5x5 core fixed. Stages 1 and 2 exclude `(3,3)`; stage 3 allows penalized `(3,3)` only if no no-corner candidate beats the current final. This pass prioritizes phi2 with weight 8 and requires `phi2 KS <= min(0.078, current_final_phi2_KS)`. Phi4 is allowed to worsen moderately up to `phi4 KS <= 0.030`; NN is guarded with `NN KS <= 0.0335`; action density, m2, m4, G_pmin_avg, and momentum stability remain hard guardrails.",
        "",
