...
OUT = LAM_ROOT / "tests/intermediate/tweak_kernel_phi2_phi4_width090_L32_to_L16_20260719"
CAND_DIR = LAM_ROOT / "kernels/candidates/tweak_phi2_phi4_width090_20260719"
FINAL = LAM_ROOT / "tests/final/final_kernel_confirmation_direct_L16_vs_blocked_L32_phi2_phi4_width090_20260719"
...
def width_target_penalty(rows: dict[str, dict[str, Any]]) -> float:
    ...
    targets = {
        ...
        "phi2": (0.90, 0.86, 0.95, 170.0),
        "phi4": (0.90, 0.86, 0.95, 190.0),
...
def objective(rows: dict[str, dict[str, Any]], tails: dict[str, dict[str, float]], mom: dict[str, float], classes: dict[str, float]) -> float:
    ...
    val = (
        ...
        + 12.0 * metric_score(rows["phi2"], mean_weight=0.08, width_weight=0.45)
        + 12.0 * metric_score(rows["phi4"], mean_weight=0.06, width_weight=0.45)
    ...
    for obs in ["phi2", "phi4"]:
        val += 1800.0 * max(0.0, tails[obs]["below_q05"] - 0.042) ** 2
        val += 2200.0 * max(0.0, tails[obs]["above_q95"] - 0.042) ** 2
        val += 2600.0 * max(0.0, tails[obs]["above_q99"] - 0.012) ** 2
        val += 450.0 * max(0.0, 0.020 - tails[obs]["below_q05"]) ** 2
...
def evaluate(name: str, classes: dict[str, float], direct_obs: dict[str, np.ndarray], evaluator: BasisEvaluator, bins: int, momentum_grid: int) -> tuple[dict[str, Any], dict[str, dict[str, Any]], dict[str, dict[str, float]], dict[str, float], np.ndarray]:
    ...
    rec["promotion_candidate"] = bool(
        ...
        and rec["phi4_above_q95"] <= 0.045
        and rec["phi2_std_ratio_blocked_over_direct"] <= 0.96
...
def write_kernel(name: str, classes: dict[str, float], rec: dict[str, Any], rows: dict[str, dict[str, Any]], tails: dict[str, dict[str, float]], mom: dict[str, float]) -> Path:
    ...
    payload = {
        ...
        "family": "phi2_phi4_width090_5x5_from_final_chosen",
...
def main() -> int:
    ...
    best_path = write_kernel("best_phi2_phi4_width090_5x5_eta_included", best_cls, best_rec, best_rows, best_tails, best_mom)
    ...
    confirm: list[tuple[str, dict[str, float]]] = [("current_final_chosen", start), ("best_phi2_phi4_width090_5x5_eta_included", best_cls)]
    ...
    write_csv(FINAL / "kernel_sum_eta_check.csv", [
        ...
        {
            "candidate": "best_phi2_phi4_width090_5x5_eta_included",
    ...
    np.savetxt(FINAL / "best_phi2_phi4_width090_eta_included_matrix.txt", best_matrix, fmt="%.16e")
    np.savetxt(FINAL / "best_phi2_phi4_width090_base_unit_sum_matrix.txt", best_matrix / ETA_SCALE, fmt="%.16e")
    ...
    for cand in ["current_final_chosen", "best_phi2_phi4_width090_5x5_eta_included"]:
        for obs in ["action_density", "phi2", "phi4", "local_kurtosis_ratio", "NN", "G_pmin_avg"]:
