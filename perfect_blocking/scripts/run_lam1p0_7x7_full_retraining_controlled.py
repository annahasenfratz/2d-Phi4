#!/usr/bin/env python3
"""Controlled from-scratch lambda=1.0 7x7 kernel retraining."""
...
OUT = LAM_ROOT / "tests/intermediate/7x7_full_retraining_controlled"
CAND_DIR = LAM_ROOT / "kernels/candidates/7x7_full_retraining_controlled"
...
def guardrail_penalty(rows: dict[str, dict[str, Any]], mom: dict[str, float], cls: dict[str, float]) -> float:
    penalty = 0.0
    for obs, limit in {
...
def total_score(rows: dict[str, dict[str, Any]], mom: dict[str, float], cls: dict[str, float]) -> float:
    return float(local_score(rows) + guardrail_penalty(rows, mom, cls))
...
def evaluate(name: str, family: str, cls: dict[str, float], direct_obs: dict[str, np.ndarray], fine: np.ndarray) -> tuple[dict[str, Any], dict[str, dict[str, Any]], dict[str, float], np.ndarray]:
    ...
    rec: dict[str, Any] = {
        ...
        "constrained_score": total_score(rows, mom, cls),
        "min_K": mom["min_K"],
        "max_K": mom["max_K"],
...
def main() -> int:
    ...
    for name, family, cls, meta in broad_specs:
        ...
        score = total_score(rows, mom, cls)
    ...
    for name, family, cls, meta in baselines + [(name, family, cls, meta) for _score, name, family, cls, meta in full_specs]:
        rec, rows, mom, matrix = evaluate(name, family, cls, direct_obs, fine)
    ...
    if best is not None:
        ...
        cand = make_candidate(
            ...
            cls,
            {
                ...
                "selection": "best staged controlled from-scratch 7x7 candidate",
    ...
    lines = [
        "# Lambda 1.0 Controlled Full 7x7 Retraining",
        "",
        "This study retrains the symmetric 7x7 orbit classes without keeping the retrained 5x5 core fixed. Stages 1 and 2 exclude `(3,3)`; stage 3 allows penalized `(3,3)` only if no no-corner candidate beats the current final.",
        "",
