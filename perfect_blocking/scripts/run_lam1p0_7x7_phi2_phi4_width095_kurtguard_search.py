#!/usr/bin/env python3
"""Constrained 7x7 no-corner lambda=1.0 width-0.95/kurtosis-guard search.

Non-promotion study.  The goal is to test whether allowing a 7x7 stencil can
make blocked L32->L16 distributions moderately narrower in phi2/phi4, with direct native
...
OUT = LAM_ROOT / "tests/intermediate/7x7_phi2_phi4_width095_kurtguard_L32_to_L16_20260719"
CAND_DIR = LAM_ROOT / "kernels/candidates/7x7_phi2_phi4_width095_kurtguard_20260719"
FINAL = LAM_ROOT / "tests/final/final_kernel_confirmation_direct_L16_vs_blocked_L32_7x7_phi2_phi4_width095_kurtguard_20260719"
...
def width_target(rows: dict[str, dict[str, Any]]) -> float:
    targets = {
        "phi2": (0.95, 0.90, 1.00, 180.0),
        "phi4": (0.95, 0.90, 1.00, 210.0),
        "action_density": (0.92, 0.87, 0.99, 120.0),
        "local_kurtosis_ratio": (0.93, 0.89, 0.99, 260.0),
...
def objective(rows: dict[str, dict[str, Any]], tails: dict[str, dict[str, float]], mom: dict[str, float], classes: dict[str, float]) -> float:
    val = (
        ...
        + 30.0 * metric_score(rows["local_kurtosis_ratio"], mean_weight=0.16, width_weight=0.85)
        + 13.0 * metric_score(rows["phi2"], mean_weight=0.08, width_weight=0.42)
        + 13.0 * metric_score(rows["phi4"], mean_weight=0.06, width_weight=0.42)
    ...
    for obs in ["phi2", "phi4"]:
        val += 1200.0 * max(0.0, tails[obs]["above_q95"] - 0.055) ** 2
        val += 1800.0 * max(0.0, tails[obs]["above_q99"] - 0.017) ** 2
        val += 900.0 * max(0.0, tails[obs]["below_q05"] - 0.058) ** 2
...
def write_kernel(name: str, classes: dict[str, float], rec: dict[str, Any], rows: dict[str, dict[str, Any]], tails: dict[str, dict[str, float]], mom: dict[str, float]) -> Path:
    ...
    payload = {
        ...
        "family": "7x7_no33_phi2_phi4_width095_kurtguard",
...
def main() -> int:
    ...
    best_path = write_kernel("best_7x7_no33_phi2_phi4_width095_kurtguard_eta_included", best_cls, best_rec, best_rows, best_tails, best_mom)
    ...
    confirm = [("current_final_5x5", current_cls), ("best_7x7_no33_phi2_phi4_width095_kurtguard_eta_included", best_cls)]
    ...
    write_csv(FINAL / "kernel_sum_eta_check.csv", [
        ...
        {"candidate": "best_7x7_no33_phi2_phi4_width095_kurtguard_eta_included", "kernel_path": str(best_path), "sum_K": float(best_matrix.sum()), "eta_scale": ETA_SCALE, "kernel_coefficients_include_eta_scale": True, **best_mom},
    ])
    np.savetxt(FINAL / "best_7x7_no33_phi2_phi4_width095_kurtguard_eta_included_matrix.txt", best_matrix, fmt="%.16e")
    np.savetxt(FINAL / "best_7x7_no33_phi2_phi4_width095_kurtguard_base_unit_sum_matrix.txt", best_matrix / ETA_SCALE, fmt="%.16e")

    cr = {(r["candidate"], r["observable"]): r for r in confirm_rows}
    ct = {(r["candidate"], r["observable"]): r for r in confirm_tails}
    lines = [
        "# Lambda 1.0 7x7 Phi2/Phi4 Width 0.95 With Kurtosis Guard Search",
        "",
    ...
    for cand in ["current_final_5x5", "best_7x7_no33_phi2_phi4_width095_kurtguard_eta_included"]:
        for obs in ["action_density", "phi2", "phi4", "local_kurtosis_ratio", "NN", "G_pmin_avg"]:
            r = cr[(cand, obs)]
            t = ct[(cand, obs)]
