...
OUT="$ROOT/perfect_blocking/perfect_blocking_lam1p0/tests/intermediate/ethan_7x7_objective_ablations"
CMD=("$PYTHON" -B "$ROOT/perfect_blocking/scripts/run_lam1p0_ethan_7x7_ablations.py" --out "$OUT")
printf 'out=%s\npython=%s\n' "$OUT" "$PYTHON"
[[ "$EXECUTE" -eq 1 ]] || exit 0
mkdir -p "$OUT/logs"
