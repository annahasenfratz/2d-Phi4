#!/usr/bin/env python3
"""Final confirmation for the lambda=1.0 7x7 no33 candidate."""
...
OUT = LAM_ROOT / "tests/final/final_kernel_confirmation_7x7_no33"
...
KERNELS = {
    "old_final_5x5": LAM_ROOT / "kernels/final/chosen_kernel.json",
    "retrained_5x5": LAM_ROOT / "kernels/candidates/systematic_training/best_retrained_5x5_full_objective_eta_included.json",
    "candidate_7x7_no33": LAM_ROOT / "kernels/candidates/7x7_from_retrained_5x5/best_7x7_from_retrained_5x5_eta_included.json",
}
...
def main() -> int:
    ...
    decision_checks = {
        "phi2_improves_vs_retrained": val("candidate_7x7_no33", "phi2", "ks_statistic") < val("retrained_5x5", "phi2", "ks_statistic"),
        "phi4_improves_vs_retrained": val("candidate_7x7_no33", "phi4", "ks_statistic") < val("retrained_5x5", "phi4", "ks_statistic"),
        "local_kurtosis_no_regress": val("candidate_7x7_no33", "local_kurtosis_ratio", "ks_statistic") <= val("retrained_5x5", "local_kurtosis_ratio", "ks_statistic"),
        "action_density_at_least_as_good_with_margin": val("candidate_7x7_no33", "action_density", "ks_statistic") <= val("retrained_5x5", "action_density", "ks_statistic") + 0.005,
        "m2_comparable": val("candidate_7x7_no33", "m2", "ks_statistic") <= val("retrained_5x5", "m2", "ks_statistic") + 0.004,
        "m4_comparable": val("candidate_7x7_no33", "m4", "ks_statistic") <= val("retrained_5x5", "m4", "ks_statistic") + 0.004,
        "G_pmin_comparable": val("candidate_7x7_no33", "G_pmin_avg", "ks_statistic") <= val("retrained_5x5", "G_pmin_avg", "ks_statistic") + 0.004,
        "dense_K_positive": float(mom_by["candidate_7x7_no33"]["min_K"]) > 0.0,
        "max_invK_bounded": float(mom_by["candidate_7x7_no33"]["max_inverse_K"]) <= 1.6,
    }
    passes = all(decision_checks.values())

    lines = [
        "# Final Confirmation: Lambda 1.0 7x7 no33 Candidate",
        "",
        "Compared old final 5x5, retrained 5x5, and best 7x7 no33 candidate on identical L16/L32 ensembles.",
        "",
        "## Full-Sample Metrics",
        "",
        "| observable | old final KS | retrained KS | 7x7 KS | old JS | retrained JS | 7x7 JS |",
        "|---|---:|---:|---:|---:|---:|---:|",
    ]
    for obs in KEY_OBS:
        lines.append(
            f"| {obs} | {val('old_final_5x5', obs, 'ks_statistic'):.6g} | {val('retrained_5x5', obs, 'ks_statistic'):.6g} | {val('candidate_7x7_no33', obs, 'ks_statistic'):.6g} | "
            f"{val('old_final_5x5', obs, 'jensen_shannon'):.6g} | {val('retrained_5x5', obs, 'jensen_shannon'):.6g} | {val('candidate_7x7_no33', obs, 'jensen_shannon'):.6g} |"
        )
    ...
    lines.extend(
        [
            ...
            "2. Copy `perfect_blocking/perfect_blocking_lam1p0/kernels/candidates/7x7_from_retrained_5x5/best_7x7_from_retrained_5x5_eta_included.json` to `kernels/final/chosen_kernel.json`.",
            "3. Regenerate `kernels/final/chosen_kernel.txt`.",
            "4. Update `kernels/final/README.md` and `perfect_blocking_lam1p0/README.md` marking the previous final 5x5 as superseded.",
        ]
    )
