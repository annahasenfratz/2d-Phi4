#!/usr/bin/env bash
# Wolff+radial L64 rethermalization, cascade replica r2.
...
SOURCE="$ROOT/perfect_blocking_upsampling/outputs/cascade_wolff_lam1p0/CASCADE-WOLFF-LAM1/L32toL64_kc0p340301_kf0p340301/N1500_r2/initialization/levels/L32toL64/checkpoints/checkpoint_sweep_0000.npz"
RUN="$ROOT/perfect_blocking_upsampling/outputs/cascade_wolff_lam1p0/CASCADE-WOLFF-LAM1/L32toL64_kc0p340301_kf0p340301/N1500_r2"
DRIVER="$ROOT/perfect_blocking_upsampling/scripts/run_lam1p0_l256_wolff_rethermalization.py"
grep -q '^WOLFF-CASCADE-L64-002,' "$ROOT/registry/runs.csv" || { echo 'missing registry entry WOLFF-CASCADE-L64-002' >&2; exit 1; }
CMD=(env PYTHONUNBUFFERED=1 "$PY" -B "$DRIVER" --source "$SOURCE" --run-dir "$RUN" --L 64 --start-index 0 --n-chains 1500 --target-sweeps "$TARGET" --checkpoint-every 25 --measurement-batch-size 50 --clusters-per-sweep 4 --seed 2026082703 --kappa 0.340301)
[[ "$RESUME" -eq 1 ]] && CMD+=(--resume)
printf 'study=CASCADE-WOLFF-LAM1\nrun_id=WOLFF-CASCADE-L64-002\n'
printf 'source=%s\nrun_dir=%s\n' "$SOURCE" "$RUN"
...
[[ "$EXECUTE" -eq 1 ]] || exit 0
[[ -f "$SOURCE" ]] || { echo "missing L64 r2 sweep-zero source; run the flow-only stage first: $SOURCE" >&2; exit 1; }
