#!/usr/bin/env bash
# Continue replica r2: L32(kappa_c=.340100)->L64(kappa_f=.340301)
# from absolute sweep 100 through absolute sweep 500.
...
CMD=(env PYTHONUNBUFFERED=1 "$PY" -B "$ROOT/perfect_blocking_upsampling/scripts/run_lam1p0_fine_hmc.py" --run-dir "$RUN" --initialization input --input-source "$INPUT" --n-chains 1500 --n-sweeps 400 --save-every 5 --batch-size 50 --hmc-batch-size 50 --measurement-batch-size 50 --divide 1 --step-size 0.05555555555555555 --leapfrog-steps 36 --seed 2026082534 --kappa 0.340301 --kappa-coarse 0.340100 --sweep-offset 100 --append --level-name L32toL64)
printf 'continuation: %s\ninput=%s\nabsolute sweeps: 100 -> 500; kappa_c=.340100, kappa_f=.340301; global tau=2, n=36\n' "$RUN" "$INPUT"
[[ "$EXECUTE" -eq 1 ]] || exit 0
