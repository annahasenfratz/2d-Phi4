#!/usr/bin/env bash
# L8 -> L16 -> L32 -> L64: 100 direct fine-HMC sweeps after every flow step.
# Active sublattice volume stays fixed at 16^2. Usage: bash $0 r1 [--execute] [--background]
set -euo pipefail
...
CONFIG="${CONFIG:-$ROOT/perfect_blocking_upsampling/run_configs/lam1p0_hmc_upscale_chain_L8to64_100each.json}"
N_CHAINS="${N_CHAINS:-1500}"; START_INDEX="${START_INDEX:-0}"
RUN="$ROOT/perfect_blocking_upsampling/outputs/hmc_upscale_chain_lam1p0/L8toL64/L8toL64_N${N_CHAINS}_start${START_INDEX}_HMCtherm100_100_100_${RUN_NUMBER}"
