#!/usr/bin/env bash
# Alternating-KL iteration-5 flow/kernel, followed by exact checkerboard MH.
# Usage: bash $0 r1 [--execute] [--background]
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"

# Current best exact-global-A/R pair from the L16->L32 alternating-KL series.
...
# Keep the established checkerboard coarse/detail MH geometry.  The flow is
# called once at sweep zero; every numbered sweep is exact local MH.
export LC="${LC:-16}"
export LF="${LF:-32}"
...
export OUTPUT_ROOT="${OUTPUT_ROOT:-$ROOT/perfect_blocking_upsampling/outputs/checkerboard_mh_lam1p0/alternatingKL5flow_kernel}"
