#!/usr/bin/env bash
# Continue the rescaled L128->L256 normalization control from sweep 100 to 200.
...
CMD=(env PYTHONUNBUFFERED=1 "$PY" -B "$ROOT/perfect_blocking_upsampling/scripts/run_lam1p0_fine_hmc.py"
  ...
  --divide 1 --step-size 0.0277777777777778 --leapfrog-steps 72 --seed 2026082106 --level-name L128toL256)
