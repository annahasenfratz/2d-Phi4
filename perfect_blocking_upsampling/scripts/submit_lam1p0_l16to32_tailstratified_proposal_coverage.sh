#!/usr/bin/env bash
# Tail-enriched conditional training plus one-sided proposal-envelope penalties.
set -euo pipefail
...
RUN_LABEL="lam1p0_L16to32_current5x5_tailstratified_proposal_coverage_N5000_$(date +%Y%m%dT%H%M%SZ)"; RUN_DIR="$REPO_ROOT/perfect_blocking_upsampling/runs/lam1p0/training/$RUN_LABEL"
...
CMD=("$PYTHON" -B "$DRIVER" --run-dir "$RUN_DIR" --source-checkpoint "$CHECKPOINT" --kernel-path "$KERNEL" --normalization-metadata "$NORMALIZATION" --source-start-index 0 --total-count 5000 --train-count 4000 --val-count 500 --test-count 500 --epochs 4 --patience 2 --eval-every 1 --exact-eval-every 2 --raw-eval-count 500 --global-chains 4 --global-sweeps 5 --local-chains 4 --local-sweeps 2 --batch-size 128 --lr 5e-6 --random-seed 2026080417 --device cpu --obs-weights action_density=0.025,phi2=0.020,phi4=0.025,local_kurtosis_ratio=0.020,NN=0.012,2nn=0.004,diag=0.004,G_pmin_avg=0 --tail-stratified-train --tail-stratified-quantile 0.10 --tail-stratified-tail-fraction 0.40 --proposal-action-lowtail-weight 0.10 --proposal-kurtosis-lowtail-weight 0.15)
...
mkdir -p "$RUN_DIR/logs"; { echo "run_id=$RUN_LABEL"; echo 'training=40% draws from the union of native low action/kurtosis deciles; loss=one-sided q01/q05 coverage'; printf 'command='; printf '%q ' "${CMD[@]}"; printf '\n'; } > "$RUN_DIR/submit_manifest.txt"
