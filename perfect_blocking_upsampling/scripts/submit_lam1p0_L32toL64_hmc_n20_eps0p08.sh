#!/usr/bin/env bash
# L32->L64: HMC trajectory eps=.08, n=20 (tau=1.6), fixed 16^2 active patch.
...
CONFIG="${CONFIG:-$ROOT/perfect_blocking_upsampling/run_configs/lam1p0_hmc_L32toL64_d4_n20_eps0p08.json}"
INPUT_L32="${INPUT_L32:-$ROOT/perfect_blocking_upsampling/outputs/fine_hmc_lam1p0/L16toL32/L16toL32_N1500_S100_start0_HMCd2_n20_eps0p08_r23/checkpoints/checkpoint_sweep_0100.npz}"
RUN="$ROOT/perfect_blocking_upsampling/outputs/hmc_upscale_chain_lam1p0/L32toL64/L32toL64_N${N_CHAINS}_S100_start${START_INDEX}_HMCd4_n20_eps0p08_${RUN_NUMBER}"
...
[[ $EXEC -eq 1 ]] || exit 0
[[ -f "$INPUT_L32" ]] || { echo "missing completed L32 source: $INPUT_L32" >&2; exit 1; }
[[ ! -e "$RUN" ]] || { echo "run directory already exists: $RUN" >&2; exit 1; }
mkdir -p "$RUN/logs"
if [[ $BG -eq 1 ]]; then
  { echo "started_at=$(date -Iseconds)"; echo "input_L32=$INPUT_L32"; printf 'command='; printf '%q ' "${CMD[@]}"; printf '\n'; } >"$RUN/logs/run.log"
