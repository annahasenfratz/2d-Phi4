#!/usr/bin/env bash
# Evaluate the epoch-1 best-NLL checkpoint from the multi-epoch distribution
# run on 5,000 samples, then write notebook-ready direct/upscaled CSVs.
set -euo pipefail
...
UPSCALE="$REPO_ROOT/perfect_blocking_upsampling/scripts/compare_lam1p0_raw_upscaled_vs_native.py"; MEASURE="$REPO_ROOT/perfect_blocking/scripts/block_and_measure.py"; MEASURE_CFG="$REPO_ROOT/perfect_blocking/perfect_blocking_lam1p0/run_configs/kernel_training_lam1p0.yaml"
CHECKPOINT="$REPO_ROOT/perfect_blocking_upsampling/runs/lam1p0/training/lam1p0_L16to32_current5x5_multiepoch_distribution_N5000_20260803T141556Z/checkpoints/checkpoint_best_nll.pt"
KERNEL="$REPO_ROOT/perfect_blocking/perfect_blocking_lam1p0/kernels/final/chosen_kernel.json"; NATIVE="$REPO_ROOT/data/configs_phi4_2d/lam1p0_kappac0p340301_L32/configs.npz"
COUNT=5000; STAMP="$(date +%Y%m%dT%H%M%SZ)"; RUN_DIR="$REPO_ROOT/perfect_blocking_upsampling/runs/lam1p0/upscaling/raw_L16toL32_current5x5_multiepoch_distribution_N${COUNT}_${STAMP}"; FIELDS="$REPO_ROOT/data/configs_phi4_2d/upscaled/lam1p0_kappac0p340301_L16_to_L32_current5x5_multiepoch_distribution_N${COUNT}_${STAMP}.npz"
...
UP=("$PYTHON" -B "$UPSCALE" --checkpoint "$CHECKPOINT" --kernel "$KERNEL" --native-l32 "$NATIVE" --output-dir "$RUN_DIR" --count "$COUNT" --start-index 0 --seed 2026080412 --batch-size 128 --label current5x5_multiepoch_distribution_raw_upscaled --upscaled-config-output "$FIELDS")
...
mkdir -p "$RUN_DIR/logs" "$RUN_DIR/observables"; { echo "checkpoint=$CHECKPOINT"; echo "generated_fields=$FIELDS"; } > "$RUN_DIR/submit_manifest.txt"
