#!/usr/bin/env bash
# L16->L32 HMC: same epsilon as the original cascade stage (.08), but 20
# leapfrog steps (trajectory length 1.6).  Starts from the same L8->L16 fields.
...
CMD=(env PYTHONUNBUFFERED=1 "$PY" -B "$ROOT/perfect_blocking_upsampling/scripts/run_lam1p0_fine_hmc.py"
  ...
  --batch-size "$BATCH_SIZE" --start-index "$START_INDEX" --step-size "$STEP_SIZE" --leapfrog-steps "$LEAPFROG_STEPS"
  --divide "$DIVIDE" --seed "$SEED" --level-name L16toL32)
printf 'run_dir=%s\ninput_L16=%s\ntrajectory_length=%s\n' "$RUN" "$L16_SOURCE" "$(awk "BEGIN {print $STEP_SIZE * $LEAPFROG_STEPS}")"
[[ $EXEC -eq 1 ]] || exit 0
[[ -f "$L16_SOURCE" ]] || { echo "missing completed L16 source: $L16_SOURCE" >&2; exit 1; }
