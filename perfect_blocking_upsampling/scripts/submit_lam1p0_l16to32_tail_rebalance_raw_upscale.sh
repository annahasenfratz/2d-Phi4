#!/usr/bin/env bash
# Generate 5,000 raw L32 fields with the gentle phi2/phi4 tail-rebalanced flow.
set -euo pipefail
...
CHECKPOINT="$REPO_ROOT/perfect_blocking_upsampling/runs/lam1p0/training/lam1p0_L16to32_current5x5_gentle_phi2_phi4_tail_rebalance_N5000_20260803T125117Z/checkpoints/checkpoint_best_nll.pt"
...
RUN_NAME="raw_L16toL32_current5x5_tail_rebalance_N${COUNT}_${STAMP}"
RUN_DIR="$REPO_ROOT/perfect_blocking_upsampling/runs/lam1p0/upscaling/$RUN_NAME"
UPSCALED_CONFIGS="$REPO_ROOT/data/configs_phi4_2d/upscaled/lam1p0_kappac0p340301_L16_to_L32_current5x5_tail_rebalance_N${COUNT}_${STAMP}.npz"
...
CMD=("$PYTHON" -B "$DRIVER"
  ...
  --label current5x5_tail_rebalance_raw_upscaled
