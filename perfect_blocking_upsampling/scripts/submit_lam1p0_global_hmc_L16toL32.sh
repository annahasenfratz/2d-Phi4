#!/usr/bin/env bash
# Global fine-field HMC rethermalization, L16 -> L32.
...
INPUT="${INPUT_L32:-$ROOT/perfect_blocking_upsampling/outputs/fine_hmc_lam1p0/L16toL32/L16toL32_N1500_S100_start0_HMCd2_n20_eps0p10_r22/checkpoints/checkpoint_sweep_0000.npz}"
RUN="$ROOT/perfect_blocking_upsampling/outputs/fine_hmc_lam1p0/global/L16toL32_N${N_CHAINS}_S${N_SWEEPS}_full_tau2_n28_eps2over28_sweep0"
CMD=("$PY" -B "$ROOT/perfect_blocking_upsampling/scripts/run_lam1p0_fine_hmc.py"
  ...
  --step-size 0.07142857142857142 --leapfrog-steps 28 --divide 1 --seed 2026081517 --level-name L16toL32)
printf 'L16->L32: N=%s sweeps=%s global HMC eps=2/28 n=28 tau=2\ninput=%s\noutput=%s\n' "$N_CHAINS" "$N_SWEEPS" "$INPUT" "$RUN"
[[ "$EXECUTE" -eq 1 ]] || exit 0
