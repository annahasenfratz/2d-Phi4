...
OUT="${OUT:-$ROOT/perfect_blocking_upsampling/outputs/global_ar_lam1p0/L8to16_alternatingKL_5x5_r1}"
...
CMD=("$PY" -B "$ROOT/perfect_blocking_upsampling/scripts/compare_lam1p0_l8to16_alternating_kl_global_ar.py" --out "$OUT" --n "$N")
printf 'out=%s\nn=%s\n' "$OUT" "$N"; [[ $EXEC -eq 1 ]] || exit 0
mkdir -p "$OUT/logs"
