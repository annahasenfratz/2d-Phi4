#!/usr/bin/env bash
# A final small continuation from the original best-NLL flow.  It emphasizes
# phi4 and local-kurtosis shape without the stronger tail rebalance that made
# the raw kurtosis distribution worse.
set -euo pipefail
...
TOTAL_COUNT=5000
SEED=2026080405
RUN_LABEL="lam1p0_L16to32_current5x5_gentle_kurtosis_phi4_shape_N${TOTAL_COUNT}_$(date +%Y%m%dT%H%M%SZ)"
...
CMD=("$PYTHON" -B "$DRIVER"
  ...
  --batch-size 128 --lr 3e-6 --random-seed "$SEED" --device cpu
  --obs-weights action_density=0.025,phi2=0.020,phi4=0.030,local_kurtosis_ratio=0.055,NN=0.012,2nn=0.004,diag=0.004,G_pmin_avg=0
  --two-sided-tail-guard --action-support-weight 0.025 --phi2-support-weight 0.008 --phi4-support-weight 0.030
  --tail-guard-std-weight 0.05 --tail-guard-quantile-weight 0.25 --tail-guard-occupancy-weight 0.5
  --tail-guard-low-occupancy-weight 0.75 --tail-guard-high-occupancy-weight 0.50
  --local-kurtosis-shape-guard --local-kurtosis-shape-weight 0.020
...
mkdir -p "$RUN_DIR/logs"
{
  ...
  printf 'tweak=phi4 weight 0.030; kurtosis observable 0.055; kurtosis shape 0.020; lr 3e-6\n'
  printf 'command='; printf '%q ' "${CMD[@]}"; printf '\n'
