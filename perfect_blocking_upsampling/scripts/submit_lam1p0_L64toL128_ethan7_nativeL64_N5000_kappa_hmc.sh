#!/usr/bin/env bash
# One member of the paired 5000-root clean L64->L128 kappa scan.
...
KAPPA_TAG="${KAPPA_F/./p}"
N_CHAINS=5000; N_SWEEPS=100; SAVE_EVERY=5; KAPPA_C=0.340301
INPUT="$ROOT/perfect_blocking_upsampling/outputs/flow_only_lam1p0/L64toL128/L64toL128_ethan7_nativeL64_N5000_sweep0_r1/levels/L64toL128/checkpoints/checkpoint_sweep_0000.npz"
NATIVE="$ROOT/data/configs_phi4_2d/lam1p0_kappac0p340301_L128/configs.npz"
RUN="$ROOT/perfect_blocking_upsampling/outputs/fine_hmc_lam1p0/global/L64toL128_ethan7_nativeL64_sweep0_kf${KAPPA_TAG}_N5000_S100_tau2_n51_eps2over51_r1"
CMD=(env PYTHONUNBUFFERED=1 "$PY" -B "$ROOT/perfect_blocking_upsampling/scripts/run_lam1p0_fine_hmc.py" --run-dir "$RUN" --initialization input --input-source "$INPUT" --native-source "$NATIVE" --n-chains "$N_CHAINS" --n-sweeps "$N_SWEEPS" --save-every "$SAVE_EVERY" --batch-size 25 --hmc-batch-size 25 --measurement-batch-size 25 --divide 1 --step-size 0.0392156862745098 --leapfrog-steps 51 --seed 2026082514 --kappa "$KAPPA_F" --kappa-coarse "$KAPPA_C" --level-name L64toL128)
printf 'shared initialization=%s\npaired clean HMC: N=5000, sweeps=100, kappa_c=%.6f -> kappa_f=%.6f\noutput=%s\n' "$INPUT" "$KAPPA_C" "$KAPPA_F" "$RUN"
[[ "$EXECUTE" -eq 1 ]] || exit 0
