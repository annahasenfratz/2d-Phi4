#!/usr/bin/env bash
# Rethermalize Ethan-7x7 L32->L64 sweep-zero fields at kappa_f=0.340330.
# The initialization is unchanged; only the physical fine HMC target differs.
...
KAPPA_C=0.340301
KAPPA_F=0.340330
INPUT="$ROOT/perfect_blocking_upsampling/outputs/hmc_upscale_chain_lam1p0/L32toL64/L32toL64_ethan7_N1500_S100_global_tau2_n36_eps2over36_r1/levels/L32toL64/checkpoints/checkpoint_sweep_0000.npz"
NATIVE="$ROOT/data/configs_phi4_2d/lam1p0_kappac0p340301_L64/configs.npz"
RUN="$ROOT/perfect_blocking_upsampling/outputs/fine_hmc_lam1p0/global/L32toL64_ethan7_sweep0_kf0p34033_N${N_CHAINS}_S${N_SWEEPS}_tau2_n36_eps2over36_r1"
...
CMD=(env PYTHONUNBUFFERED=1 "$PY" -B "$ROOT/perfect_blocking_upsampling/scripts/run_lam1p0_fine_hmc.py"
  ...
  --divide 1 --step-size 0.05555555555555555 --leapfrog-steps 36 --seed 2026082502
  --kappa "$KAPPA_F" --kappa-coarse "$KAPPA_C" --level-name L32toL64)

printf 'L32->L64 sweep-zero rethermalization: N=%s, sweeps=%s\n' "$N_CHAINS" "$N_SWEEPS"
printf 'initialization=%s\n' "$INPUT"
printf 'HMC target: lambda=1, kappa_c=%.6f -> kappa_f=%.6f; global volume, tau=2, n=36, eps=2/36\n' "$KAPPA_C" "$KAPPA_F"
printf 'output=%s\n' "$RUN"
