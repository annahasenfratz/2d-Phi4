#!/usr/bin/env bash
# Gentle exact cold-mixture diagnostic after the 15% cold component over-broadened the proposal.
set -euo pipefail
...
LABEL="raw_L32toL64_current5x5_cold_mixture_envelope_w0p05_T0p90_N5000_$STAMP"
RUN_DIR="$REPO_ROOT/perfect_blocking_upsampling/runs/lam1p0/upscaling/$LABEL"
FIELD="$REPO_ROOT/data/configs_phi4_2d/upscaled/lam1p0_kappac0p340301_L32_to_L64_current5x5_cold_mixture_envelope_w0p05_T0p90_N5000_$STAMP.npz"
...
CMD=("$PYTHON" -B "$DRIVER" --checkpoint "$CKPT" --kernel "$KERNEL" --native-source "$NATIVE" --coarse-source "$COARSE" --coarse-lattice 32 --fine-lattice 64 --output-dir "$RUN_DIR" --count 5000 --seed 2026080423 --batch-size 64 --latent-mixture-weight 0.05 --latent-temperature 0.90 --label cold_mixture_envelope_w0p05_T0p90 --upscaled-config-output "$FIELD")
...
mkdir -p "$RUN_DIR/logs"; { echo "run_id=$LABEL"; echo 'proposal=exact mixture: 95% standard flow + 5% latent-temperature-0.90 flow; mixture logq is evaluated exactly'; printf 'command='; printf '%q ' "${CMD[@]}"; printf '\n'; } > "$RUN_DIR/submit_manifest.txt"
