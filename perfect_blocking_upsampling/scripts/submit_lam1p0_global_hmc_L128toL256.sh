...
RUN="$ROOT/perfect_blocking_upsampling/outputs/fine_hmc_lam1p0/global/L128toL256_N${N_CHAINS}_S${N_SWEEPS}_full_tau2_n27_eps2over27_sweep0"
CMD=("$PY" -B "$ROOT/perfect_blocking_upsampling/scripts/run_lam1p0_fine_hmc.py"
  ...
  --step-size 0.07407407407407407 --leapfrog-steps 27 --divide 1
  --batch-size 8 --hmc-batch-size 8 --measurement-batch-size 8
  --seed 2026081526 --level-name L128toL256)
printf 'L128->L256: N=%s sweeps=%s global HMC eps=2/27 n=27 tau=2; HMC/measurement batch=8\ninput=%s\noutput=%s\n' "$N_CHAINS" "$N_SWEEPS" "$INPUT" "$RUN"
[[ "$EXECUTE" -eq 1 ]] || exit 0
