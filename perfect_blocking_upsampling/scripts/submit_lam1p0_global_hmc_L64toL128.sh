...
RUN="$ROOT/perfect_blocking_upsampling/outputs/fine_hmc_lam1p0/global/L64toL128_N${N_CHAINS}_S${N_SWEEPS}_full_tau2_n33_eps2over33_sweep0"
CMD=("$PY" -B "$ROOT/perfect_blocking_upsampling/scripts/run_lam1p0_fine_hmc.py"
  ...
  --step-size 0.06060606060606061 --leapfrog-steps 33 --divide 1
  --batch-size 50 --hmc-batch-size 50 --measurement-batch-size 50
  --seed 2026081522 --level-name L64toL128)
printf 'L64->L128: N=%s sweeps=%s global HMC eps=2/33 n=33 tau=2\ninput=%s\noutput=%s\n' "$N_CHAINS" "$N_SWEEPS" "$INPUT" "$RUN"
[[ "$EXECUTE" -eq 1 ]] || exit 0
