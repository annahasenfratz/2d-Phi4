#!/usr/bin/env bash
# Unshifted control: the same clean native-L32 Ethan-7x7 L64 sweep-zero
# fields rethermalized at kappa_f=kappa_c=0.340301.
...
KAPPA_C=0.340301; KAPPA_F=0.340301
...
RUN="$ROOT/perfect_blocking_upsampling/outputs/fine_hmc_lam1p0/global/L32toL64_ethan7_nativeL32_sweep0_kf0p340301_N${N_CHAINS}_S${N_SWEEPS}_tau2_n36_eps2over36_r1"
...
printf 'paired clean control: kappa_c=%.6f -> kappa_f=%.6f; global tau=2, n=36, eps=2/36\n' "$KAPPA_C" "$KAPPA_F"
printf 'output=%s\n' "$RUN"
[[ "$EXECUTE" -eq 1 ]] || exit 0
