#!/usr/bin/env bash
# Rethermalize the exact direct-native-L16, highcorr-5x5 flow samples at L32.
# Uses full-field HMC: tau=2, eps=2/28, n=28.  This was the recent L16->L32
# calibration, with observed acceptance 0.863; it is the requested ~85% setup.
...
SEED="${SEED:-2026081813}"
INPUT="$ROOT/perfect_blocking_upsampling/outputs/flow_input_audit_lam1p0/highcorr5_pureNLL_N10000_direct_native_L16_N5000_20260818_with_fields/sweep0_flow_phi.npz"
RUN="$ROOT/perfect_blocking_upsampling/outputs/fine_hmc_lam1p0/global/L16toL32_highcorr5_pureNLL_directnative_N${N_CHAINS}_S${N_SWEEPS}_tau2_n28_eps2over28_20260818"
...
printf 'run_dir=%s\ninput=%s\nN=%s sweeps=%s save_every=%s\nfull-field HMC: eps=2/28, n=28, tau=2; calibrated acceptance=0.863\n' \
