...
DEFAULT_OUT = PKG / "outputs" / "shape_parametric_sampler_validation" / "controlled_coarse_patch_restart_32to64_patch4_patch6"
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    requested_settings = set(getattr(args, "_settings", [(p, n) for p in args.patch_sizes for n in args.coarse_passes]))
    for patch_size in args.patch_sizes:
        for n_passes in args.coarse_passes:
            if (patch_size, n_passes) not in requested_settings:
                continue
    ...
    write_csv(out / "controlled_32to64_coarse_site_updates.csv", site_rows)
    write_csv(out / "controlled_32to64_patch_moves.csv", patch_rows)
    write_csv(out / "fine_AR_after_controlled_32to64_patch.csv", fine_rows)
    write_csv(out / "local_operator_changes_after_controlled_32to64_patch.csv", local_rows)
    write_csv(out / "controlled_32to64_patch_summary.csv", summary_rows)
    summary = {
        "status": "completed",
        ...
        "settings": sorted([list(x) for x in requested_settings]),
...
def write_report(out: Path, summary: dict[str, Any], summary_rows: list[dict[str, Any]], local_rows: list[dict[str, Any]]) -> None:
    lines = [
        "# Controlled coarse patch restart 32->64, P=4 and P=6",
        "",
        "This diagnostic starts the fresh controlled L32->L64 validation path. A selected coarse patch is updated by site-by-site Metropolis targeting `S_c`; every coarse site proposal has its own A/R using local `Delta S_c`; the latent/detail field `z` is held fixed; the proposed fine field is reconstructed and tested with the existing fine/transformed A/R logweight.",
        "",
        "## Setup",
        "",
    ...
    lines += [
        "",
        "## Comparison To 16->32 Controlled Baseline",
        "",
        "| setting | 16->32 fine A/R baseline | 32->64 fine A/R | drop |",
        "|---|---:|---:|---:|",
    ]
    baseline = {(4, 4): 0.875, (6, 2): 0.773438}
    for r in summary_rows:
        key = (int(r["patch_size"]), int(r["n_coarse_passes"]))
        if key in baseline:
            base = baseline[key]
            lines.append(f"| P={key[0]}, passes={key[1]} | {base:.6g} | {r['fine_AR_acceptance']:.6g} | {base - r['fine_AR_acceptance']:.6g} |")
    lines += [
        "",
    ...
    p62 = by_key.get((6, 2))
    if p44:
        lines.append(f"1. P=4, 4 passes: fine A/R is `{p44['fine_AR_acceptance']:.6g}` with fine-log-accept std `{p44['fine_log_accept_std']:.6g}`.")
    if p62:
        lines.append(f"2. P=6, 2 passes: fine A/R is `{p62['fine_AR_acceptance']:.6g}` with fine-log-accept std `{p62['fine_log_accept_std']:.6g}`.")
    if p44:
        lines.append(f"3. Compared with the 16->32 controlled baseline P=4, 4 passes (`0.875`), the 32->64 fine A/R drop is `{0.875 - p44['fine_AR_acceptance']:.6g}`.")
    if p62:
        lines.append(f"4. Compared with the 16->32 controlled baseline P=6, 2 passes (`0.773438`), the 32->64 fine A/R drop is `{0.773438 - p62['fine_AR_acceptance']:.6g}`.")
    lines.append("5. This is the controlled validation path. Previous uncontrolled coarse-patch L32->L64 tests are not production-algorithm validation.")
    lines.append("6. Heatbath was not implemented here; the diagnostic uses site Metropolis only.")
    lines.append("")
    (out / "CONTROLLED_COARSE_PATCH_RESTART_32TO64_PATCH4_PATCH6_REPORT.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    ap.add_argument("--settings", type=str, nargs="+", default=["4:4", "6:2"], help="Patch/pass settings as P:passes.")
    ap.add_argument("--patch-sizes", type=int, nargs="+", default=None, help="Deprecated; use --settings.")
    ap.add_argument("--coarse-passes", type=int, nargs="+", default=None, help="Deprecated; use --settings.")
    ...
    for item in args.settings:
        ...
        parsed_settings.append((int(p_str), int(passes_str)))
    args.patch_sizes = []
    args.coarse_passes = []
    for patch_size, n_passes in parsed_settings:
        if patch_size not in args.patch_sizes:
            args.patch_sizes.append(patch_size)
        if n_passes not in args.coarse_passes:
            args.coarse_passes.append(n_passes)
    args._settings = parsed_settings
    summary = run(args)
