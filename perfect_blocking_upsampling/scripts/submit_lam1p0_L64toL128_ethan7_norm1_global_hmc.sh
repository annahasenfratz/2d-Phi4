...
# L64->L128 flow sample, but apply 2^(-eta/2) to its physical fine field.
...
CMD=(env PYTHONUNBUFFERED=1 "$PY" -B "$ROOT/perfect_blocking_upsampling/scripts/run_lam1p0_fine_hmc.py"
  ...
  --divide 1 --step-size 0.0392156862745098 --leapfrog-steps 51 --seed 2026082104 --level-name L64toL128)
printf 'run_dir=%s\nsource=%s\ninitial_field_scale=%s = 2^(-eta/2), eta=0.25\nHMC=global full-volume, tau=2, n=51, eps=2/51\n' "$RUN" "$SOURCE" "$SCALE"
[[ "$EXECUTE" -eq 1 ]] || exit 0
[[ -f "$SOURCE" ]] || { echo "missing L128 sweep-zero source: $SOURCE" >&2; exit 1; }
