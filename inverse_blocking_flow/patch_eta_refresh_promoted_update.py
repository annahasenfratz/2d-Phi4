"""Promoted coarse-patch updates with local eta refresh on the patch."""
...
SCHEDULES = [
    {"name": "A_same_eta", "eta_halo": None, "theta": None},
    {"name": "B_patch_theta005", "eta_halo": 0, "theta": 0.05},
    {"name": "C_patch_theta010", "eta_halo": 0, "theta": 0.10},
    {"name": "D_patch_theta020", "eta_halo": 0, "theta": 0.20},
    {"name": "E_halo_theta005", "eta_halo": 1, "theta": 0.05},
    {"name": "F_halo_theta010", "eta_halo": 1, "theta": 0.10},
]
...
def detailed_observables(phi: torch.Tensor, params: Phi4Params) -> dict[str, float | list[float] | None]:
    ...
    return {
        ...
        "C_over_C0": [None if c0 is None else float(v / c0) for v in ct],
        "P00": p00,
        "P10": p10,
        "P01": p01,
        "P11": p11,
...
def eta_patch_mask(size: int, y0: int, x0: int, patch_size: int, halo: int, device: torch.device) -> torch.Tensor:
    mask = torch.zeros((size, size), dtype=torch.bool, device=device)
    for dy in range(-halo, patch_size + halo):
        for dx in range(-halo, patch_size + halo):
            mask[(y0 + dy) % size, (x0 + dx) % size] = True
    return mask


@torch.no_grad()
def inner_patch_mcmc_with_origin(
    psi: torch.Tensor,
    params_c: Phi4Params,
    *,
    patch_size: int,
    ...
    size = psi.shape[-1]
    y0 = int(torch.randint(0, size - patch_size + 1, (1,), generator=generator, device=psi.device).item())
    x0 = int(torch.randint(0, size - patch_size + 1, (1,), generator=generator, device=psi.device).item())
    accepts = 0
    proposals = 0
    ...
    return psi_new, patch_sq, accepts, proposals, y0, x0


def eta_patch_refresh(
    eta: torch.Tensor,
    mask: torch.Tensor,
    theta: float,
    generator: torch.Generator,
    ...
    eta_prop = eta.clone()
    noise = torch.randn(eta.shape, dtype=eta.dtype, device=eta.device, generator=generator)
    mix_old = math.cos(theta)
    mix_new = math.sin(theta)
    eta_prop[:, :, mask] = mix_old * eta[:, :, mask] + mix_new * noise[:, :, mask]
    return eta_prop


def current_logJ(flow, eta: torch.Tensor, psi: torch.Tensor, conditioning_mode: str) -> tuple[torch.Tensor, torch.Tensor]:
...
def run_chain(
    ...
    if start_mode == "equilibrium_start":
        ...
        logJ = flow.standard_normal_logprob(eta) - logq
        ...
        u, logJ = current_logJ(flow, eta, psi, conditioning_mode)
        logq = flow.standard_normal_logprob(eta) - logJ
        phi = soft_reconstruct(psi[:, 0], u)
    ...
    for attempt in range(1, args.n_promoted_attempts_per_config + 1):
        ...
        patch_mask = eta_patch_mask(psi.shape[-1], y0, x0, patch_size, int(eta_halo or 0), device=psi.device)
        eta_prop = eta_patch_refresh(eta, patch_mask, float(theta or 0.0), generator) if eta_halo is not None else eta.clone()
        u_prop, logJ_new = current_logJ(flow, eta_prop, psi_prop, conditioning_mode)
        ...
        logp_old_H = -0.5 * eta[:, :, patch_mask].square().sum(dim=(1, 2))
        logp_new_H = -0.5 * eta_prop[:, :, patch_mask].square().sum(dim=(1, 2))
        loga = -(sf_new - sf) - (k_new - kernel) + (sc_new - sc) + (logJ_new - logJ) + (logp_old_H - logp_new_H)

        if schedule["name"] == "A_same_eta" and attempt == 1:
            theta0 = eta_patch_refresh(eta, patch_mask, 0.0, generator)
            u0, logJ0 = current_logJ(flow, theta0, psi_prop, conditioning_mode)
            same_eta_loga = -(sf_new - sf) - (k_new - kernel) + (sc_new - sc) + (logJ0 - logJ)
            theta_zero_max_error = float((loga - same_eta_loga).abs().max().detach().cpu())
        ...
        promoted_logj.extend((logJ_new - logJ).detach().cpu().tolist())
            ...
            promoted_rows.append(
                {
                    ...
                    "logq_old_minus_logq_new": float((logJ_new - logJ)[j].detach().cpu() + (logp_old_H - logp_new_H)[j].detach().cpu()),
        ...
        if accept.any():
            ...
            logq[accept] = flow.standard_normal_logprob(eta_prop[accept]) - logJ_new[accept]

        if eta_halo is not None:
            eta_accepts += int(accept.sum())
            eta_total += int(accept.numel())
            for j in range(int(accept.numel())):
                eta_rows.append(
                    {
                        "accepted": float(bool(accept[j])),
                        ...
                        "minus_Delta_K_alpha": float((-(k_new - kernel)[j]).detach().cpu()),
                        "plus_Delta_S_c": float((sc_new - sc)[j].detach().cpu()),
                        "logq_old_minus_logq_new": float(((logJ_new - logJ) + (logp_old_H - logp_new_H))[j].detach().cpu()),
                        "total_logA": float(loga[j].detach().cpu()),
                        "Delta_S_f": float((sf_new - sf)[j].detach().cpu()),
                        "Delta_K_alpha": float((k_new - kernel)[j].detach().cpu()),
                        "Delta_S_c": float((sc_new - sc)[j].detach().cpu()),
    ...
    promoted_ar = promoted_accepts / float(promoted_total)
    eta_ar = eta_accepts / float(eta_total) if eta_total else None
    ...
    eta_patch_sites = int(((2 * int(eta_halo or 0)) + patch_size) ** 2) if eta_halo is not None else 0
    return {
        "start_mode": start_mode,
        "schedule": schedule,
        "kappa_f": float(setup["kappa_f"]),
        "patch_size": patch_size,
        "eta_patch_sites": eta_patch_sites,
        "sigma_psi": sigma,
        "n_inner_hits": n_inner_hits,
        "promoted_acceptance": promoted_ar,
        "eta_patch_acceptance": 0.0 if eta_ar is None else eta_ar,
        "accepted_coarse_sites_per_attempt": promoted_ar * float(patch_size * patch_size),
        ...
        "accepted_eta_patch_sites_per_attempt": 0.0 if eta_ar is None else eta_ar * float(eta_patch_sites),
        "inner_coarse_acceptance": inner_accepts / float(inner_proposals),
        "logA": stats(promoted_loga),
        "Delta_S_f": stats(promoted_dsf),
        "Delta_S_c": stats(promoted_dsc),
        "Delta_K": stats(promoted_dk),
        "logq_or_logJ_term": stats(promoted_logj),
        "logA_component_split": split_component_summary(promoted_rows),
        "eta_logA_component_split": split_component_summary(eta_rows),
        "theta_zero_same_eta_logA_max_error": theta_zero_max_error,
        "history": history,
...
def load_controls(args: argparse.Namespace) -> dict[str, object]:
    out: dict[str, object] = {}
    same_eta_path = args.output_dir / "same_eta_plus_eta_refresh_summary.json"
    if same_eta_path.exists():
        data = json.loads(same_eta_path.read_text())
        runs = data.get("runs", [])
        by_name = {r["schedule"]["name"]: r for r in runs}
        out["global_eta_refresh"] = by_name.get("E_every50_theta010")
        out["same_eta_baseline"] = by_name.get("A_no_eta_refresh")
    ...
    if restricted_path.exists():
        ...
        out["restricted_detail"] = {run["name"]: run for run in data.get("runs", [])}
    return out
...
def write_report(path: Path, summary: dict[str, object]) -> None:
    ...
    lines = [
        "# Patch Eta-Refresh Promoted Update",
        "",
        "State is represented as `(psi, eta)`. Promoted updates refresh `eta` only on the selected coarse patch or patch-plus-halo region.",
        "",
        "## Movement Ranking",
        "",
        "| rank | start | schedule | theta | eta halo | A/R | eta A/R | accepted D_patch/attempt | accepted eta sites/attempt | final xi/L | local err | IR err | total err |",
        "|---:|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for i, row in enumerate(ranked, 1):
        lines.append(
            f"| {i} | {row['start_mode']} | {row['schedule']['name']} | {'' if row['schedule']['theta'] is None else row['schedule']['theta']} | "
            f"{'' if row['schedule']['eta_halo'] is None else row['schedule']['eta_halo']} | {row['promoted_acceptance']:.6g} | "
            f"{'' if row['eta_patch_acceptance'] is None else f'{row['eta_patch_acceptance']:.6g}'} | {row['accepted_D_patch_per_attempt']:.6g} | "
            f"{'' if row['accepted_eta_patch_sites_per_attempt'] is None else f'{row['accepted_eta_patch_sites_per_attempt']:.6g}'} | "
            f"{row['final_observables']['xi_2nd_over_L']} | {row['final_aggregate_errors']['local']:.6g} | {row['final_aggregate_errors']['IR']:.6g} | {row['final_aggregate_errors']['total']:.6g} |"
        )
    ...
    lines.extend(
        [
            ...
            "## Theta-Zero Check",
            "",
            "| start | schedule | max |theta=0 - same-eta| |",
            "|---|---|---:|",
        ]
    )
    for row in runs:
        lines.append(
            f"| {row['start_mode']} | {row['schedule']['name']} | {row['theta_zero_same_eta_logA_max_error']:.6g} |"
        )
    ...
    if controls.get("same_eta_baseline") is not None:
        row = controls["same_eta_baseline"]
        lines.append(
            f"| same_eta_baseline | previous benchmark | {row['final_observables']['xi_2nd_over_L']} | {row['final_aggregate_errors']['local']:.6g} | {row['final_aggregate_errors']['IR']:.6g} | {row['final_aggregate_errors']['total']:.6g} | "
            f"{row['autocorrelation']['M']['tau_int_initial_positive']} | {row['autocorrelation']['S_density']['tau_int_initial_positive']} | {row['autocorrelation']['phi2']['tau_int_initial_positive']} | {row['autocorrelation']['lowest_momentum_mode']['tau_int_initial_positive']} |"
        )
    if controls.get("global_eta_refresh") is not None:
        row = controls["global_eta_refresh"]
        lines.append(
            f"| global_eta_refresh_E50 | previous benchmark | {row['final_observables']['xi_2nd_over_L']} | {row['final_aggregate_errors']['local']:.6g} | {row['final_aggregate_errors']['IR']:.6g} | {row['final_aggregate_errors']['total']:.6g} | "
            f"{row['autocorrelation']['M']['tau_int_initial_positive']} | {row['autocorrelation']['S_density']['tau_int_initial_positive']} | {row['autocorrelation']['phi2']['tau_int_initial_positive']} | {row['autocorrelation']['lowest_momentum_mode']['tau_int_initial_positive']} |"
        )
    for name, row in controls.get("local_mcmc", {}).items():
        final = row["history"][-1]["observables"]["mean"]
        comp = row.get("final_aggregate_errors", {"local": None, "IR": None, "total": None})
        ac = row["autocorrelation"] if "autocorrelation" in row else row["autocorrelation_summary"]
        lines.append(
    ...
    for name, row in controls.get("restricted_detail", {}).items():
        final = row.get("final", {})
        lines.append(
    ...
    lines.extend(
        [
            ...
            "Patch-only eta refresh should be judged by whether it improves local decorrelation without pushing the IR away from the cluster reference.",
        ]
    )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def plot_outputs(path: Path, summary: dict[str, object]) -> None:
    ...
    with PdfPages(path) as pdf:
        ...
        axes[0, 1].bar(labels, [0.0 if r["eta_patch_acceptance"] is None else r["eta_patch_acceptance"] for r in runs])
        axes[0, 1].set_ylabel("eta patch A/R")
...
def main() -> None:
    ...
    ref_tensor = load_cluster_or_reference(args, 0.320)
    ref = bootstrap_observables_local(ref_tensor, Phi4Params(0.320, 1.0), args.diagnostic_bootstrap, args.seed + 320)
    controls = load_controls(args)
    for key in ("same_eta_baseline", "global_eta_refresh"):
        row = controls.get(key)
        if isinstance(row, dict) and row.get("history"):
            final_obs = row["history"][-1]["observables"]
            row["final_observables"] = final_obs["mean"]
            row["final_comparison"] = compare_observables(final_obs, ref)
    ...
    for start_index, start_mode in enumerate(("upscaled_start", "equilibrium_start")):
        for schedule_index, schedule in enumerate(SCHEDULES):
            ...
            row = run_chain(
                ...
                psi0,
                metadata,
                {"kappa_f": 0.320, "patch_size": PATCH_SIZE, "sigma_psi": SIGMA_PSI, "n_inner_hits": N_INNER_HITS},
                schedule,
                args,
                start_mode,
                gen,
            ...
            print(
                ...
                f"etaA={'' if row['eta_patch_acceptance'] is None else f'{row['eta_patch_acceptance']:.4g}'} "
                f"D={row['accepted_D_patch_per_attempt']:.4g} xi={row['final_observables']['xi_2nd_over_L']}",
                flush=True,
            )
    ...
    ref_phi = ref_tensor[: args.n_configs].to(device)
    ...
    summary = {
        "setup": {
            ...
            "eta_refresh_mode": "patch-only on P or P+halo",
            "patch_size": PATCH_SIZE,
            "sigma_psi": SIGMA_PSI,
            "n_inner_hits": N_INNER_HITS,
            "cost_note": "Promoted cost units count 200 coarse hits + one flow eval approximated as 256 units + one fine action eval approximated as 256 units.",
        },
        "controls": controls,
        "runs": runs,
        "local_mcmc": local_controls,
        "cost_units": cost_units(N_INNER_HITS),
    }

    summary_path = args.output_dir / "patch_eta_refresh_promoted_update_summary.json"
    report_path = args.output_dir / "patch_eta_refresh_promoted_update_report.md"
    plots_path = args.output_dir / "patch_eta_refresh_promoted_update_plots.pdf"
