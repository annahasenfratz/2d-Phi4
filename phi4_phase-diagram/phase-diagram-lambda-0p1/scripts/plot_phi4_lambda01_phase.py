#!/usr/bin/env python3
"""Summarize phi4 lambda=0.1 phase-diagram scan."""
...
def main() -> None:
    ...
    parser.add_argument("--broad", type=Path, default=Path("outputs/phi4_lambda01_cluster_l16_l24_l32_curves.csv"))
    parser.add_argument("--refined", type=Path, default=Path("outputs/phi4_lambda01_cluster_l16_l24_l32_refined_curves.csv"))
    parser.add_argument("--output-png", type=Path, default=Path("outputs/phi4_lambda01_l16_l24_l32_chi_binder.png"))
    parser.add_argument("--output-pdf", type=Path, default=Path("outputs/phi4_lambda01_l16_l24_l32_chi_binder.pdf"))
    parser.add_argument("--output-json", type=Path, default=Path("outputs/phi4_lambda01_l16_l24_l32_chi_binder.json"))
    ...
    crossings = {
        "0.300": {
            "16-24": crossing(refined[(16, 0.300)], refined[(24, 0.300)], args.min_ess),
            "24-32": crossing(refined[(24, 0.300)], refined[(32, 0.300)], args.min_ess),
            "16-32": crossing(refined[(16, 0.300)], refined[(32, 0.300)], args.min_ess),
        },
        "0.305": {
            "16-24": crossing(refined[(16, 0.305)], refined[(24, 0.305)], args.min_ess),
            "24-32": crossing(refined[(24, 0.305)], refined[(32, 0.305)], args.min_ess),
            "16-32": crossing(refined[(16, 0.305)], refined[(32, 0.305)], args.min_ess),
        },
        "broad_0.300": {
            "16-24": crossing(broad[(16, 0.300)], broad[(24, 0.300)], args.min_ess),
            "24-32": crossing(broad[(24, 0.300)], broad[(32, 0.300)], args.min_ess),
            "16-32": crossing(broad[(16, 0.300)], broad[(32, 0.300)], args.min_ess),
        },
    }
    ...
    ax.set_title(r"$\lambda=0.1$ phi4 $\chi_{|m|}$")
    ...
    ax = axes[1]
    for L in [16, 24, 32]:
        pts = selected(refined[(L, 0.300)], "binder_u4", args.min_ess)
        ax.plot([r["kappa"] for r in pts], [r["binder_u4"] for r in pts], "o-", color=COLORS[L], linewidth=2.0, markersize=2.6, label=f"L={L}, k0=0.300")
    for c in crossings["0.300"].values():
        if c["method"] == "linear":
            ax.axvline(c["kappa_crossing"], color="#777777", linestyle=":", linewidth=1.0)
    ax.text(0.02, 0.98, "Binder crossings\n" + "\n".join(f"{pair}: {c['kappa_crossing']:.6f} ({c['method']})" for pair, c in crossings["0.300"].items()), transform=ax.transAxes, va="top", fontsize=9, bbox={"facecolor": "white", "alpha": 0.78, "edgecolor": "#cccccc"})
    ax.set_title(r"$\lambda=0.1$ phi4 Binder, $k_0=0.300$")
    ax.set_xlabel(r"$\kappa$")
    ax.set_ylabel(r"$U_4$")
    ax.grid(True, color="#dddddd")
    ax.legend(fontsize=9)
    fig.suptitle(r"2D phi4 phase diagram check, $\lambda=0.1$", fontsize=13)
    ...
    out = {
        ...
        "notes": ["lambda=0.1 shows larger reweighting-center dependence than lambda=0.5 or 1."],
    }
