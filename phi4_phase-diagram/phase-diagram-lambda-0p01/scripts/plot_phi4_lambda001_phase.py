#!/usr/bin/env python3
"""Summarize phi4 lambda=0.01 phase-diagram scan."""
...
def crossing(curve_a: list[dict[str, float]], curve_b: list[dict[str, float]], min_ess: float) -> dict[str, float | str]:
    ...
    for (k0, d0), (k1, d1) in zip(diffs, diffs[1:]):
        if d0 * d1 < 0:
            return {"kappa_crossing": float(k0 - d0 * (k1 - k0) / (d1 - d0)), "method": "linear", "bracket": [k0, k1]}
...
def main() -> None:
    ...
    parser.add_argument("--broad", type=Path, default=Path("outputs/phi4_lambda001_cluster_l16_l24_l32_curves.csv"))
    parser.add_argument("--refined", type=Path, default=Path("outputs/phi4_lambda001_cluster_l16_l24_l32_refined_curves.csv"))
    parser.add_argument("--output-png", type=Path, default=Path("outputs/phi4_lambda001_l16_l24_l32_chi_binder.png"))
    parser.add_argument("--output-pdf", type=Path, default=Path("outputs/phi4_lambda001_l16_l24_l32_chi_binder.pdf"))
    parser.add_argument("--output-json", type=Path, default=Path("outputs/phi4_lambda001_l16_l24_l32_chi_binder.json"))
    ...
    exponent, log_amp = np.polyfit(np.log(L_arr), np.log(chi_arr), 1)
    crossings = {
        "0.261": {
            "16-24": crossing(refined[(16, 0.261)], refined[(24, 0.261)], args.min_ess),
            "24-32": crossing(refined[(24, 0.261)], refined[(32, 0.261)], args.min_ess),
            "16-32": crossing(refined[(16, 0.261)], refined[(32, 0.261)], args.min_ess),
        },
        "0.262": {
            "16-24": crossing(refined[(16, 0.262)], refined[(24, 0.262)], args.min_ess),
            "24-32": crossing(refined[(24, 0.262)], refined[(32, 0.262)], args.min_ess),
            "16-32": crossing(refined[(16, 0.262)], refined[(32, 0.262)], args.min_ess),
        },
        "broad_0.260": {
            "16-24": crossing(broad[(16, 0.260)], broad[(24, 0.260)], args.min_ess),
            "24-32": crossing(broad[(24, 0.260)], broad[(32, 0.260)], args.min_ess),
            "16-32": crossing(broad[(16, 0.260)], broad[(32, 0.260)], args.min_ess),
        },
    }
    ...
    for (L, k0), curve in sorted(refined.items()):
        ...
        ax.plot([r["kappa"] for r in pts], [r["susceptibility_abs_centered"] for r in pts], color=COLORS[L], linestyle=STYLES[round(k0, 3)], linewidth=1.8, label=f"L={L}, k0={k0:.3f}")
    ax.text(0.02, 0.98, "mean chi peaks\n" + "\n".join(f"L={L}: {averaged[str(L)]['kappa_peak_mean']:.5f}" for L in [16, 24, 32]), transform=ax.transAxes, va="top", fontsize=9, bbox={"facecolor": "white", "alpha": 0.78, "edgecolor": "#cccccc"})
    ax.set_title(r"$\lambda=0.01$ phi4 $\chi_{|m|}$")
    ...
    ax = axes[1]
    for L in [16, 24, 32]:
        pts = selected(refined[(L, 0.261)], "binder_u4", args.min_ess)
        ax.plot([r["kappa"] for r in pts], [r["binder_u4"] for r in pts], "o-", color=COLORS[L], linewidth=2.0, markersize=2.6, label=f"L={L}, k0=0.261")
    for c in crossings["0.261"].values():
        if c["method"] == "linear":
            ax.axvline(c["kappa_crossing"], color="#777777", linestyle=":", linewidth=1.0)
    ax.text(0.02, 0.98, "Binder crossings\n" + "\n".join(f"{pair}: {c['kappa_crossing']:.6f} ({c['method']})" for pair, c in crossings["0.261"].items()), transform=ax.transAxes, va="top", fontsize=9, bbox={"facecolor": "white", "alpha": 0.78, "edgecolor": "#cccccc"})
    ax.set_title(r"$\lambda=0.01$ phi4 Binder, $k_0=0.261$")
    ax.set_xlabel(r"$\kappa$")
    ax.set_ylabel(r"$U_4$")
    ax.grid(True, color="#dddddd")
    ax.legend(fontsize=9)
    fig.suptitle(r"2D phi4 phase diagram check, $\lambda=0.01$", fontsize=13)
    ...
    out = {
        ...
        "height_power_fit": {"exponent": float(exponent), "amplitude": float(np.exp(log_amp))},
        "binder_crossings": crossings,
        "notes": [
            "lambda=0.01 susceptibility peak locations are tight near kappa=0.2609 on L=16,24,32.",
            "The chi peak-height exponent and Binder crossings are not yet Ising-clean; Binder crossings shift by about 1e-3 between kappa0=0.261 and 0.262 refined ensembles.",
        ],
    }
