#!/usr/bin/env python3
"""Summarize phi4 lambda=0.5 phase-diagram sanity scan."""
...
def main() -> None:
    ...
    parser.add_argument("--broad", type=Path, default=Path("outputs/phi4_lambda05_cluster_l16_l24_l32_curves.csv"))
    parser.add_argument("--refined", type=Path, default=Path("outputs/phi4_lambda05_cluster_l16_l24_l32_refined_curves.csv"))
    parser.add_argument("--output-png", type=Path, default=Path("outputs/phi4_lambda05_l16_l24_l32_chi_binder.png"))
    parser.add_argument("--output-pdf", type=Path, default=Path("outputs/phi4_lambda05_l16_l24_l32_chi_binder.pdf"))
    parser.add_argument("--output-json", type=Path, default=Path("outputs/phi4_lambda05_l16_l24_l32_chi_binder.json"))
    ...
    crossings = {
        "0.34": {
            "16-24": crossing(broad[(16, 0.34)], broad[(24, 0.34)], args.min_ess),
            "24-32": crossing(broad[(24, 0.34)], broad[(32, 0.34)], args.min_ess),
            "16-32": crossing(broad[(16, 0.34)], broad[(32, 0.34)], args.min_ess),
        },
        "0.36": {
            "16-24": crossing(broad[(16, 0.36)], broad[(24, 0.36)], args.min_ess),
            "24-32": crossing(broad[(24, 0.36)], broad[(32, 0.36)], args.min_ess),
            "16-32": crossing(broad[(16, 0.36)], broad[(32, 0.36)], args.min_ess),
        },
    }
    ...
    for (L, k0), curve in sorted(refined.items()):
        ...
        ax.plot([r["kappa"] for r in pts], [r["susceptibility_abs_centered"] for r in pts], color=COLORS[L], linestyle=STYLES[round(k0, 3)], linewidth=1.9, label=f"L={L}, k0={k0:.3f}")
    ax.text(
        0.02,
        0.98,
        "mean chi peaks\n" + "\n".join(f"L={L}: {averaged[str(L)]['kappa_peak_mean']:.5f}" for L in [16, 24, 32]),
        transform=ax.transAxes,
        va="top",
        fontsize=9,
        bbox={"facecolor": "white", "alpha": 0.78, "edgecolor": "#cccccc"},
    )
    ax.set_title(r"$\lambda=0.5$ phi4 $\chi_{|m|}$")
    ...
    ax.text(
        0.02,
        0.98,
        "Binder crossings\n" + "\n".join(f"{pair}: {c['kappa_crossing']:.6f}" for pair, c in crossings["0.34"].items()),
        transform=ax.transAxes,
        va="top",
        fontsize=9,
        bbox={"facecolor": "white", "alpha": 0.78, "edgecolor": "#cccccc"},
    )
    ax.set_title(r"$\lambda=0.5$ phi4 Binder, $k_0=0.340$")
    ax.set_xlabel(r"$\kappa$")
    ax.set_ylabel(r"$U_4$")
    ax.grid(True, color="#dddddd")
    ax.legend(fontsize=9)
    fig.suptitle(r"2D phi4 phase diagram check, $\lambda=0.5$", fontsize=13)
